...
tvpqr <- function(Y,X,Xout=NULL,bt_init=NULL,mode="qr",p=0.5,tvp="dhs",sv=FALSE,cons.cons=FALSE,fhorz=0,
                  nburn=1000,nsave=1000,thinfac=1,quiet=FALSE,pred.dsp=F,pred.sv=F){
  ...
  # some checks
  if((!pred.dsp & fhorz>0) | (!pred.sv & fhorz>0)){
    stop("Check forecast settings.")
  }
  if(!is.null(Xout)){
...
tvpqr.grid <- function(Y,X,Xout=NULL,p=seq(0.05,0.95,by=0.05),cpu=1,tvp="dhs",sv=TRUE,cons.cons=FALSE,fhorz=0,
  ...
  if(fhorz>0){
    fcst_store <- array(NA,dim=c(nsave,P,T))
    fcstsig2_store <- array(NA,dim=c(nsave,P,T))
    for(pp in 1:P){
      fcst_store[,paste0("p",p.grid[pp]*100),,] <- p.list[[pp]]$fcst
      sig2_store[,paste0("p",p.grid[pp]*100),] <- p.list[[pp]]$fcstsig2
    }
  }
  return(list("bt"=bt_store,"sig2"=sig2_store,
              "shrink"=p.list[[pp]]$shrink,
              "fcst"=p.list[[pp]]$fcst,"fcstsig2"=p.list[[pp]]$fcstsig2))
}
