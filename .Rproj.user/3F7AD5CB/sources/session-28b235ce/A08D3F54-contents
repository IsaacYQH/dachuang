...
trnct <- F
if(trnct){
  start = which(as.Date(data_raw$Dates)=="2004-10-01")
...
dates <- as.character(data$Dates)[-1:-2]
...
# X <- as.matrix(cbind(intercept=rep(1,nrow(data)-1), data[-1,2]))
...
for(sv in c(TRUE,FALSE)){
  ...
  est <- tvpqr.grid(Y=Yt,X=Xt,Xout=NULL,p=grid.p,cpu=cpu,tvp=prior,sv=sv,fhorz=0,
                    nburn=nburn,nsave=nsave,thinfac=thinfac,out="mcmc",pred.dsp=F,pred.sv=F)
  bt_store <- est$bt
  sig2_store <- est$sig2
  if(sv){
